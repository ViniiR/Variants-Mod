...
public class ModEntities {
    ...
    public static final EntityType<Icid> ICID = register("icid", EntityType.Builder.of(Icid::new, MobCategory.MONSTER).sized(0.6F, 1.95F).eyeHeight(1.74F).passengerAttachments(2.0125F).ridingOffset(-0.7F).clientTrackingRange(8).notInPeaceful());
    public static final EntityType<MuddyZombie> MUDDY_ZOMBIE = register("muddy_zombie", EntityType.Builder.of(MuddyZombie::new, MobCategory.MONSTER).sized(0.6F, 1.95F).eyeHeight(1.74F).passengerAttachments(2.0125F).ridingOffset(-0.7F).clientTrackingRange(8).notInPeaceful());

    public static final EntityType<SoulSkeleton> SOUL_SKELETON = register("soul_skeleton", EntityType.Builder.of(SoulSkeleton::new, MobCategory.MONSTER).sized(0.6F, 1.99F).eyeHeight(1.74F).ridingOffset(-0.7F).clientTrackingRange(8).notInPeaceful());

    public static final EntityType<WarpedEnderman> WARPED_ENDERMAN = register("warped_enderman", EntityType.Builder.of(WarpedEnderman::new, MobCategory.MONSTER).sized(0.6F, 2.9F).eyeHeight(2.55F).passengerAttachments(2.80625F).clientTrackingRange(8).notInPeaceful());

    public static final EntityType<PaleSnowGolem> PALE_SNOW_GOLEM = register("pale_snow_golem", EntityType.Builder.of(PaleSnowGolem::new, MobCategory.MISC).immuneTo(Blocks.POWDER_SNOW).sized(0.7F, 1.9F).eyeHeight(1.7F).clientTrackingRange(8));

    public static final EntityType<SoulBlaze> SOUL_BLAZE = register("soul_blaze", EntityType.Builder.of(SoulBlaze::new, MobCategory.MONSTER).fireImmune().sized(0.6F, 1.8F).clientTrackingRange(8).notInPeaceful());

    public static final EntityType<FrozenGuardian> FROZEN_GUARDIAN = register("frozen_guardian", EntityType.Builder.of(FrozenGuardian::new, MobCategory.MONSTER).sized(0.85F, 0.85F).eyeHeight(0.425F).passengerAttachments(0.975F).clientTrackingRange(8).notInPeaceful());
